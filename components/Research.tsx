...
export default function Research() {
  ...
  return (
    <section id="research" className="relative border-t border-edge bg-surface py-28 transition-colors duration-500 md:py-40">
      <div className="mx-auto max-w-shell px-6">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {researchAreas.map((a, i) => (
            <Reveal key={a.title} delay={(i % 3) * 0.09}>
              <div className="card-lift group h-full rounded-2xl border border-edge bg-card p-7">
                ...
                <p className="mt-5 font-mono text-2xs uppercase tracking-wider text-faint">{a.pubs}</p>
              </div>
            </Reveal>
          ))}
        </div>
            ...
            const inner = (
              <div className="group grid gap-2 py-6 transition-colors sm:grid-cols-[7rem,1fr,auto] sm:items-baseline sm:gap-6">
                ...
                <span className="hidden font-mono text-2xs text-faint sm:block">
                  {p.status === "in submission" ? "in submission" : p.status}
                </span>
              </div>
            );
